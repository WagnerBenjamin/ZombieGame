...
public class old_PlayerScript : MonoBehaviour
{
    ...
    void GetMovement()
    {
        if(isGrounded)
        {
            ...
            moveVelocity = (transform.right * zAxis + transform.forward * xAxis).normalized * moveSpeed;
    ...
    {
        CController.Move(moveVelocity * Time.fixedDeltaTime);
    ...
    private void ReactToWhatsInFrontOfPlayer()
    {
        ...
        {
            if (objectInFrontOfPlayer.TryGetComponent(out WallWeaponScript wallWeapon))
            {
